...
export const getGscConnection = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    ...
    // A connection row surviving doesn't mean the grant still works — the
    // token can be revoked/expired without the row changing, which used to
    // show a false "Connected" pill. Check it live.
    const health = connection
      ? await GscService.checkGrantHealth({
